...
async function loadAnalytics() {
      ...
      modelMetrics.innerHTML = `
        <div class="metric-cards">
          ...
          <div class="metric-card metric-card--confidence">
            ...
            <div class="metric-card__value">${ms.metrics.yield_oob ? ms.metrics.yield_oob.toFixed(4) : '—'}</div>
            <div class="metric-card__sub">Out-of-bag cross-validation</div>
          </div>
        </div>
      `;
    } else {
    ...
    splitChart.innerHTML = `
      <div class="card">
        <div class="card__header">
          ...
          <span class="tag">${typeof total === 'number' ? total.toLocaleString() : total} Total Records</span>
        </div>
        <div style="display:flex;height:24px;border-radius:var(--r-full);overflow:hidden;margin-bottom:var(--s3)">
          <div style="width:70%;background:var(--sage-600);display:flex;align-items:center;justify-content:center;color:white;font-size:var(--t-xs);font-weight:600">Train (70% - ${typeof train === 'number' ? train.toLocaleString() : train})</div>
        ...
        <div style="font-size:var(--t-xs);color:var(--muted)">
          Partitioned by unique inbred parent line ID to prevent cross-environment data leakage.
        </div>
      </div>
    `;
  }
    ...
    if (ds) {
      datasetStats.innerHTML = `
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:var(--s4)">
          <div>
            <div style="font-size:var(--t-xs);color:var(--muted)">Total Inbred Lines</div>
          ...
          <div>
            ...
            <div style="font-size:var(--t-xl);font-weight:700;color:var(--sage-900)">${ds.n_snp_markers || '300'} SNPs</div>
          </div>
          <div>
            <div style="font-size:var(--t-xs);color:var(--muted)">Evaluation Sites</div>
  ...
  if (traitDist) {
    traitDist.innerHTML = `
      <div class="card mt-4">
        <div class="card__title mb-4">Target Trait Distribution Overview</div>
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:var(--s4)">
          <div class="card" style="background:var(--bg-surface)">
            <div style="font-size:var(--t-xs);color:var(--muted);text-transform:uppercase">Drought Tolerance</div>
            <div style="font-weight:600;margin-top:4px">High / Medium / Low</div>
            <div style="font-size:var(--t-xs);color:var(--sage-700);margin-top:2px">Balanced multi-class distribution</div>
          </div>
          <div class="card" style="background:var(--bg-surface)">
            <div style="font-size:var(--t-xs);color:var(--muted);text-transform:uppercase">Disease Resistance</div>
            <div style="font-weight:600;margin-top:4px">Resistant / Moderate / Susceptible</div>
            <div style="font-size:var(--t-xs);color:var(--coral);margin-top:2px">Stratified phenotypic sampling</div>
          </div>
          <div class="card" style="background:var(--bg-surface)">
            <div style="font-size:var(--t-xs);color:var(--muted);text-transform:uppercase">Grain Quality</div>
            <div style="font-weight:600;margin-top:4px">Grade A / Grade B / Grade C</div>
            <div style="font-size:var(--t-xs);color:var(--sage-700);margin-top:2px">Calibrated probability scoring</div>
          </div>
        </div>
      </div>
    `;
